...
public class TodidListFrame extends JFrame implements ActionListener{
	...
	public TodidListFrame(String dolist) {
		...
		String[] colNames= {"종료날짜", "제목", "내용","걸린기간"};
		...
		table.addPropertyChangeListener((e)->{
			...
			if(e.getPropertyName() == "tableCellEditor" && !table.isEditing()) {
				...
				int diddate=(int)model.getValueAt(selectedIndex, 0);
				String title =(String)model.getValueAt(selectedIndex, 1);
				String content =(String)model.getValueAt(selectedIndex, 2);
				int howlong=(int)model.getValueAt(selectedIndex, 3);
				//수정할 회원의 정보보를 객체에 담고
				TodidListDto dto =new TodidListDto(diddate,title,content,howlong);
				//DB에 수정 반영하기
				new TodidListDao().update(dto);
				//선택된 포커스 clear
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("add")) {
			//입력한 번호, 이름, 주소를 읽어온다.
			String diddate = inputDate.getText();
			String title=inputTitle.getText();
			String content=inputContent.getText();
			String howlong =inputHowlong.getText();
			//MemberDto 객체에 회원 한명의 정보를 담는다.
			dto.setDiddate(Integer.parseInt(diddate));
			dto.setTitle(title);
			dto.setContent(content);
			dto.setHowlong(Integer.parseInt(howlong));
			
			//회원 한명의 정보를 DB에 저장하기
			new TodidListDao().insert(dto);
			...
			dto = list.get(row);
			int diddate = dto.getDiddate();
			new TodidListDao().delete(diddate);
	...
	public void refreshTable() {
		//DB 저장된 목록을 읽어온다.
		list =new TodidListDao().getList();
		//현재 테이블에 출력된 모든 내용을 삭제
		model.setRowCount(0);
		//반복문 돌면서 테이블 row 추가하기
		for(TodidListDto tmp:list) {
			//MemberDto 에 담긴 내용을 Object[] 에 담은 다음 
			Object[] row= {tmp.getDiddate(), tmp.getTitle(), tmp.getContent(), tmp.getHowlong()};
			//row 를 추가한다.
			model.addRow(row);
		}
	}
	...
	public static void main(String[] args) {
		JFrame f=new TodidListFrame("완료한 일 목록");
